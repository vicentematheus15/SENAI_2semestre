...
-- CRIANDO UM TRIGGER QUE MOSTRE O LOG QUANDO HOUVER ALTERAÇÃO DE SALÁRIO
-- O LOG MOSTRA QUEM FEZ A ALTERAÇÃO, SALARIO ANTIGO E NOVO, QUEM FEZ A ALTERAÇÃO E QUANDO FOI FEITA A ALTERAÇÃO

--CRIA A TABELA QUE VAI GUARDAR AS INFOS DO LOG
CREATE TABLE LOG_USER(
	COD_LOG SERIAL PRIMARY KEY,
	COD_FUNC INT,
	SAL_ANT NUMERIC(10, 2),
	SAL_ATUAL NUMERIC(10, 2),
	DATA_MOD TIMESTAMP,
	LOGIN VARCHAR(50)
);

--GARANTE ACESSO NOVAMENTE AO GRUPO TI EM TABELAS, SEQUENCIAS E FUNCOES
GRANT ALL ON ALL TABLES IN SCHEMA PUBLIC TO TI;
GRANT ALL ON ALL SEQUENCES IN SCHEMA PUBLIC TO TI;
GRANT ALL ON ALL FUNCTIONS IN SCHEMA PUBLIC TO TI;

--CRIA A FUNCAO QUE VAI RETORNAR O TRIGGER
CREATE OR REPLACE FUNCTION LOG_RH()
RETURNS TRIGGER AS $$
DECLARE USUARIO VARCHAR(50);
BEGIN
	SELECT SESSION_USER INTO USUARIO;
	INSERT INTO LOG_USER (COD_FUNC, SAL_ANT, SAL_ATUAL, DATA_MOD,LOGIN)
	VALUES(NEW.MATRICULA, OLD.SALARIO, NEW.SALARIO, NOW(), USUARIO);
	RETURN NEW;
END;
$$ LANGUAGE PLPGSQL;
...
ON FUNCIONARIO
FOR EACH ROW
EXECUTE FUNCTION LOG_RH();
	
--ATUALIZA O SALARIO DE UM FUNCIONARIO PARA TESTAR
UPDATE FUNCIONARIO SET SALARIO = 10000.00
WHERE MATRICULA = 4;

--MOSTRA OS DADOS QUE FORAM REGISTRADOS NO LOG
